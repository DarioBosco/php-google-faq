...
<head>
	...
	<title>Google Policies FAQ con PHP</title>
</head>
		...
		foreach ($faqs as $faq) {
			...
			foreach ($faq["answers"] as $answer) {
				if (is_array($answer)) {
					...
					foreach ($answer as $firstList) {
						echo '<li>';
						if (is_array($firstList)) {
							foreach ($firstList as $secondList) {
								if (is_array($secondList)) {
									echo '<ol type="a">';
									foreach ($secondList as $thirdList) {
										echo '<li>' . $thirdList . '</li>';
									}
									echo '</ol>';
								} else {
									echo $secondList;
